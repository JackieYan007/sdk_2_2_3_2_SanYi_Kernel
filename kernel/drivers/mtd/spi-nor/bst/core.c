...
int bst_spi_nor_write_reg(struct spi_nor *nor, u8 opcode,int len)
{
	int ret;
			...
			size_t *retlen, u_char *buf)
{
	...
	while (len) {
		...
		ret = spi_nor_read_data(nor, addr, FIFOSIZE, buf);
		if (ret == 0) {
			/* We shouldn't see 0-length reads */
			ret = -EIO;
			goto read_err;
...
static int spi_nor_init_params(struct spi_nor *nor)
{
	...
	if ((nor->info->flags & (SPI_NOR_DUAL_READ | SPI_NOR_QUAD_READ | SPI_NOR_OCTAL_READ)) &&
...
static void spi_nor_shutdown(struct spi_mem *spimem)
{
	...
	bst_spi_nor_write_reg(nor,0x66,0);
	bst_spi_nor_write_reg(nor,0x99,0);
	// if(nor->controller_ops->write_reg != NULL){
    //     nor->controller_ops->write_reg(nor, SPINOR_OP_EX4B, NULL, 0);
    //     udelay(500);
    //     nor->controller_ops->write_reg(nor, 0x66, NULL, 0);
    //     udelay(1000);
    //     nor->controller_ops->write_reg(nor, 0x99, NULL, 0);
    // }
	spi_nor_restore(nor);
}
...
static const struct spi_device_id spi_nor_dev_ids[] = {
	...
	{"mx25l25635e"},{"mx66l51235l"},{ "mt35xu512" },
	{"n25q064"},	{"n25q128a11"},	{"n25q128a13"},	{"n25q512a"},
	{"s25fl256s1"},	{"s25fl512s"},	{"s25sl12801"},	{"s25fl008k"},
	{"s25fl064k"},
	{"sst25vf040b"},{"sst25vf016b"},{"sst25vf032b"},{"sst25wf040"},
	{"m25p40"},	{"m25p80"},	{"m25p16"},	{"m25p32"},{"w25q256jw"},
	{"m25p64"},	{"m25p128"},
