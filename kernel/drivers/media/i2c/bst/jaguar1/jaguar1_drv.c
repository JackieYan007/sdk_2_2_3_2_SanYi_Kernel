...
unsigned int mclk = 3; //0:756 1:594 2:378 3:1242
module_param(mclk, uint, S_IRUGO);
unsigned int lane = 4; //2 or 4
module_param(lane, uint, S_IRUGO);
unsigned int chn = 4; 
module_param(chn, uint, S_IRUGO);
unsigned int init = 0; 
module_param(init, uint, S_IRUGO);
unsigned int fmt = 1;  //0:960H;1:720P 2:1080P 3:960P 4:SH720
module_param(fmt, uint, S_IRUGO);
unsigned int ntpal = 0; 
...
unsigned int bit8 = 1; //
module_param(bit8, uint, S_IRUGO);
...
static void jaguar1_video_nrt_control(unsigned char ch)
{
	gpio_i2c_write(0x60, 0xFF, 0x11);
	gpio_i2c_write(0x60, 0x18+(ch%4)*0x20, 0x05);  //nrt on, default is 0x04
	gpio_i2c_write(0x60, 0x19, 0x3F);     //如果通道是25fps的,丢掉5帧
	gpio_i2c_write(0x60, 0x1A, 0xFF);
	gpio_i2c_write(0x60, 0x1B, 0xFF);
	gpio_i2c_write(0x60, 0x1C, 0xE0);
//		gpio_i2c_write(0x60, 0x19, 0x3F);  ////如果通道是30fps的,丢掉10帧
//		gpio_i2c_write(0x60, 0x1A, 0xFF);
//		gpio_i2c_write(0x60, 0x1B, 0xFC);
//		gpio_i2c_write(0x60, 0x1C, 0x00);
}
...
int jaguar1_init(struct i2c_client *client, video_init_all *param)
{
	jaguar1_client = client;
	video_decoder_init();
	vd_set_all(param);
	return 0;
}
...
long jaguar1_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	{	
		 ...
		 * Video auto
		 *===============================================================================================*/
		case IOC_VDEC_VIDEO_AUTO_VFC_GET:
			if(copy_from_user(&stVideoInfo, argp, sizeof(nc_video_s)))
				printk(" IOC_VDEC_AUTO_VFC_GET error\n");
			jaguar1_video_auto_vfc_get(&stVideoInfo);
			cpy2usr_ret = copy_to_user(argp, &stVideoInfo, sizeof(nc_video_s));
			break;
		case IOC_VDEC_VIDEO_AUTO_NOVIDEO_SET:
			if(copy_from_user(&stVideoInfo, argp, sizeof(nc_video_s)))
				printk(" IOC_VDEC_AUTO_NOVIDEO_SET error\n");
			jaguar1_video_auto_novideo_set(&stVideoInfo);
			break;
		case IOC_VDEC_VIDEO_AUTO_VIDEO_SET:
			if(copy_from_user(&stVideoInfo, argp, sizeof(nc_video_s)))
				printk(" IOC_VDEC_AUTO_NOVIDEO_SET error\n");
			jaguar1_video_auto_video_set(&stVideoInfo);
			break;
		/*===============================================================================================
...
{
	if(gpio_request(SUB_RESET_GPIO, "rst_jaguar1") != 0)
	{
		printk("error : reset gpio request\n");
	}

	if(gpio_export(SUB_RESET_GPIO, 1) < 0)
	{
		printk("error : reset gpio export\n");
	}

	if(gpio_direction_output(SUB_RESET_GPIO, 1) != 0)
	{
		printk("error : reset gpio direction set\n");
	}

	gpio_set_value(SUB_RESET_GPIO, 0);
	msleep(1000);
	gpio_set_value(SUB_RESET_GPIO, 1);
	msleep(1);

	gpio_free(SUB_RESET_GPIO);
}
#endif
...
static int __init jaguar1_module_init(void)
{
	...
	if(init)
	{
		for(ch=0;ch<jaguar1_cnt*4;ch++)
		{
			...
			switch(fmt)
			{
				...
				break;
				case 2:
					sVideoall.ch_param[ch].format = AHD20_1080P_30P+ntpal;
				break;
				case 3:
	...
	//gpio_i2c_write(jaguar1_i2c_addr[0], 0xff, 0x20);
	//gpio_i2c_write(jaguar1_i2c_addr[0], 0x00, 0x22);  //0x11/22/44/88
