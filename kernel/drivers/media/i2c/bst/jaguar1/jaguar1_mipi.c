...
void disable_channel_AD(int dev_num, unsigned char ch)
{
	unsigned char val_1x97, val_1x98;
	gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0xFF, 0x00);
	gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0x00+ch%4, 0x11);  //bit0 1: disable, 0: enable
	
	gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0xFF, 0x01);
	val_1x97 = gpio_i2c_read(jaguar1_i2c_addr[dev_num], 0x97);
	val_1x97 &= ~(1<<(ch%4));   //bit[3:0] each bit stands for one channel. 1:enable, 0: disable
	gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0x97, val_1x97);
	val_1x98 = gpio_i2c_read(jaguar1_i2c_addr[dev_num], 0x98);
	val_1x98 |= (1<<(ch%4));   //bit[3:0] each bit stands for one channel. 0:enable, 1: disable
	gpio_i2c_write(jaguar1_i2c_addr[dev_num], 0x98, val_1x98);
	

	printk("[DRV]disable_channel_AD ch[%d] val_1x97[%x], val_1x98[%x]\n", ch, val_1x97, val_1x98);
}
