...
enum MVE_IDLE_STATE
{
    ...
    IDLE_STATE_IDLE,      /**< Session has received two idle messages in a row without the client sending
...
struct mve_state
{
    ...
    enum mve_hw_state hw_state;                 /**< Track the hw state */
    enum mve_hw_state pending_hw_state;         /**< Pending hardware state that will be requested
    ...
    enum mve_flush quick_flush_state;           /**< The current quick flush state */
    enum mve_flush quick_flush_target;          /**< When the current quick flush state
 ...
 */
mve_error mve_session_enqueue_flush_buffers(struct mve_session *session,
                                            uint32_t flush);
 ...
 * @return MVE error code.
 */
mve_error mve_session_enqueue_state_change(struct mve_session *session,
                                           enum mve_hw_state state);
