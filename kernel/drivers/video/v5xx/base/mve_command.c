 ...
 */
static mve_error mve_command_create_session_helper(void *data,
                                                   struct file *filep)
{
...
static const char *mve_command_to_string(uint32_t cmd)
{
    ...
    if (MVE_DEBUG_FIRMWARE_HUNG_SIMULATION < cmd)
    {
...
struct mve_response *mve_command_execute(struct mve_command_header *header,
                                         void *data,
                                         struct file *filep)
{
#define MVE_LOG_COMMAND(_severity)                                                                                   \
    ...
    switch (header->cmd)
    {
        case MVE_CREATE_SESSION:
            if (NULL != session)
            {
                /* Multiple sessions for one file descriptor is not allowed! */
                ret->error = MVE_ERROR_INSUFFICIENT_RESOURCES;
                MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session,
                                      MVE_LOG_ERROR,
                                      session,
            ...
            break;
        case MVE_DESTROY_SESSION:
            MVE_LOG_COMMAND(MVE_LOG_INFO);
            mve_session_destroy(session);
            break;
        case MVE_ACTIVATE_SESSION:
        {
            bool res;
            uint32_t *version;
            ...
            MVE_LOG_COMMAND(MVE_LOG_INFO);
            fw_secure_desc = (struct mve_fw_secure_descriptor *)data;
        ...
        case MVE_ENQUEUE_FLUSH_BUFFERS:
        {
        ...
        case MVE_ENQUEUE_STATE_CHANGE:
        {
            enum mve_hw_state state = ((uint32_t *)data)[0];
        ...
        case MVE_GET_EVENT:
        {
            ...
            if (NULL == event)
            {
                ret->error = MVE_ERROR_TIMEOUT;
            }
            else
            {
                ret->size = sizeof(struct mve_event_header) + event->size;
                ret->data = event;
            }
            break;
        }
        case MVE_SET_PARAMETER:
        {
            ret->error = mve_session_set_paramconfig(session,
                                                     ...
                                                     &ret->firmware_error,
                                                     PARAMETER);
            if (ret->error == MVE_ERROR_NONE)
            {
                MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            }
            break;
        }
        case MVE_GET_PARAMETER:
        {
        ...
        case MVE_REGISTER_BUFFER:
        {
            uint32_t port_index = *((uint32_t *)data);
            struct mve_buffer_userspace *descriptor =
                (struct mve_buffer_userspace *)(data + sizeof(uint32_t));

            /* Verify that the user and kernel space versions of the mve_buffer_userspace
             * match in at least size */
            WARN_ON(header->size != sizeof(uint32_t) + sizeof(struct mve_buffer_userspace));

            MVE_LOG_COMMAND(MVE_LOG_INFO);
            if (1 < port_index)
            {
                ret->error = MVE_ERROR_BAD_PORT_INDEX;
                MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session,
                                      MVE_LOG_ERROR,
                                      session,
        ...
        case MVE_UNREGISTER_BUFFER:
        {
            mve_buffer_handle_t buffer_id = *((mve_buffer_handle_t *)data);
        ...
        case MVE_FILL_THIS_BUFFER:
        {
            struct mve_buffer_details *param = (struct mve_buffer_details *)data;

            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_session_buffer_enqueue(session, param, false);
            break;
        }
        case MVE_EMPTY_THIS_BUFFER:
        {
            struct mve_buffer_details *param = (struct mve_buffer_details *)data;
        ...
        case MVE_NOTIFY_REF_FRAME_RELEASE:
        {
            mve_buffer_handle_t buffer_id = *((mve_buffer_handle_t *)data);
        ...
        case MVE_REQUEST_MAX_FREQUENCY:
        {
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
        ...
        case MVE_READ_HW_INFO:
        {
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->data = MVE_RSRC_MEM_CACHE_ALLOC(sizeof(struct mve_hw_info), GFP_KERNEL);
            if (NULL == ret->data)
            {
                ret->error = MVE_ERROR_INSUFFICIENT_RESOURCES;
                MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session,
            ...
            {
                struct mve_hw_info *hw_info;
                hw_info = (struct mve_hw_info *)ret->data;
                ret->size = sizeof(struct mve_hw_info);
        ...
        case MVE_DEBUG_WRITE_REGISTER:
            ...
            break;
        case MVE_DEBUG_INTERRUPT_COUNT:
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
        ...
        case MVE_DEBUG_FIRMWARE_HUNG_SIMULATION:
        {
    ...
    if (ret->error != MVE_ERROR_NONE)
    {
        ...
        if ((ret->error == MVE_ERROR_TIMEOUT) || (ret->error == MVE_ERROR_NOT_READY))
        {
