...
struct session_enqueue_request
{
    ...
    struct mve_buffer_details param;
};
 ...
 */
static enum mve_event_code response_code_to_mve_event(uint32_t response_code)
{
 ...
 * @return MVE_ERROR_NONE on success or a suitable OMX error code on failure.
 */
static mve_error enqueue_message(struct mve_session *session,
                                 uint32_t code,
                                 uint32_t size,
                                 void *data)
{
...
/**
 * Called when the state of the HW has changed (MVE_HW_STATE_STOPPED <-> MVE_HW_STATE_RUNNING). Note that
 ...
 */
static void change_session_hw_state(struct mve_session *session, enum mve_hw_state state)
{
    WARN_ON(MVE_HW_STATE_STOPPED != state && MVE_HW_STATE_RUNNING != state);

    session->state.hw_state = state;
    session->state.pending_hw_state = state;
}

static void send_event_to_userspace(struct mve_session *session, struct mve_event_header *event)
{
...
{
    struct mve_event_header *event;
    struct mve_buffer_userspace *buffer;
    enum mve_event_code event_code;

    event_code = (false != empty_this_buffer) ? MVE_EVENT_INPUT : MVE_EVENT_OUTPUT;
    event = mve_queue_create_event(event_code, sizeof(struct mve_buffer_userspace));
    if (NULL == event)
    {
        return;
    }

    buffer = (struct mve_buffer_userspace *)event->data;
 ...
 * @return OMX error code describing the error. MVE_ERROR_NONE on success.
 */
static mve_error enqueue_job(struct mve_session *session)
{
...
static void enqueue_pending_buffers(struct mve_session *session, struct list_head *list, bool empty_this_buffer)
{
    ...
    list_for_each_safe(pos, next, list)
    {
        ...
        if (MVE_ERROR_TIMEOUT == ret)
        {
            break;
        }
        else if (MVE_ERROR_NONE != ret)
        {
 ...
 */
static void process_generic_event(struct mve_session *session, struct mve_event_header *event)
{
    bool delete_event = false;

    switch (event->code)
    {
        case MVE_EVENT_OUTPUT: /* Intentional fall-through! */
        case MVE_EVENT_INPUT:
        {
            ...
            if (MVE_EVENT_OUTPUT == event->code)
            {
            ...
            {
                if ((session->state.quick_flush_target & MVE_FLUSH_OUTPUT_PORT &&
                     buffer_client->port_index == MVE_PORT_INDEX_OUTPUT) ||
                    (session->state.quick_flush_target & MVE_FLUSH_INPUT_PORT &&
                     buffer_client->port_index == MVE_PORT_INDEX_INPUT))
                {
                    ...
                    /* Do not pass MVE_EVENT_OUTPUT/INPUT to userspace when
                     * performing a quick flush. */
                    return;
                }
                else
                {
                    struct mve_event_header *usr_event;
                    struct mve_buffer_userspace *buf;
                    mve_error res;
                    ...
                    usr_event = mve_queue_create_event(event->code, sizeof(struct mve_buffer_userspace));
                    if (NULL == usr_event)
                    {
                        return;
                    }

                    buf = (struct mve_buffer_userspace *)usr_event->data;
        ...
        case MVE_EVENT_INPUT_FLUSHED:
        case MVE_EVENT_OUTPUT_FLUSHED:
        {
            if (0 != session->state.quick_flush_target)
            {
                if (MVE_EVENT_INPUT_FLUSHED == event->code)
                {
                    session->state.quick_flush_state |= MVE_FLUSH_INPUT_PORT;
                }
                else if (MVE_EVENT_OUTPUT_FLUSHED == event->code)
                {
                    session->state.quick_flush_state |= MVE_FLUSH_OUTPUT_PORT;
                }

                if ((session->state.quick_flush_target & MVE_FLUSH_ALL_PORTS) ==
                    session->state.quick_flush_state)
                {
                    ...
                    list_for_each_safe(pos, next, &session->quick_flush_buffers)
                    {
                        ...
                        if (0 != (session->state.quick_flush_target & MVE_FLUSH_QUICK_SET_INTERLACE))
                        {
        ...
        case MVE_EVENT_STATE_CHANGED:
        {
            change_session_hw_state(session, *((enum mve_hw_state *)event->data));
            if (STATE_SWITCHING_OUT != session->state.state)
            {
                response_received(session);
            }
        ...
        case MVE_EVENT_PROCESSED:
            ...
            break;
        case MVE_EVENT_IDLE:
            if (IDLE_STATE_PENDING == session->state.idle_state)
            {
            ...
            {
                /* The first idle message doesn't mean the FW is idle. Generate
                 * an IRQ and if the FW responds with a new idle message then we
                 * know that it really is idle. */
                session->state.idle_state = IDLE_STATE_PENDING;
                mver_irq_signal_mve((mver_session_id)session);
            }
            return;
        /* By default, just pass the message on to user space */
        case MVE_EVENT_ERROR:
        {
            struct event_error
            {
                uint32_t reason;
            ...
            if (e != NULL && e->reason == 9 && !strcmp(e->message, "Watchdog"))
            {
        ...
        case MVE_EVENT_ALLOC_PARAMS:
        case MVE_EVENT_SEQUENCE_PARAMS: /* Intentional fallthrough */
        {
            /* Just forward these messages to userspace */
            break;
        }
        case MVE_EVENT_REF_FRAME_RELEASED:
        {
            struct mve_buffer_client *buffer_client;
            ...
            if (NULL != buffer_client)
            {
                ...
                struct mve_event_header *usr_event;
                usr_event = mve_queue_create_event(event->code, sizeof(mve_buffer_handle_t));
                if (NULL == usr_event)
                {
                    return;
                }

                *(mve_buffer_handle_t *)usr_event->data = buffer_client->buffer->info.buffer_id;
        ...
        case MVE_EVENT_BUFFER_PARAM:
        {
            ...
            if (NULL != p && MVE_EVENT_BUFFER_PARAM_TYPE_DPB_HELD_FRAMES == p->type)
            {
        ...
        case MVE_EVENT_FW_TRACE_BUFFERS:
        {
...
static void handle_mve_message(struct mve_session *session)
{
    ...
    while (NULL != data)
    {
        ...
        if (NULL != event)
        {
            ...
            switch (event_code)
            {
                case MVE_EVENT_GET_PARAMCONFIG:
                case MVE_EVENT_SET_PARAMCONFIG:
...
mve_error mve_session_handle_rpc_mem_alloc(struct mve_session *session, int fd)
{
    ...
    if (MVE_ERROR_NONE == err)
    {
        ...
        if (fd < 0)
        {
            err = MVE_ERROR_INSUFFICIENT_RESOURCES;
            goto error;
        }
        ...
        info.allocator = MVE_BUFFER_ALLOCATOR_DMABUF;
        info.handle = fd;
        ...
        if (NULL != pages)
        {
            ...
            if (MVE_COM_MEM_REGION_PROTECTED == rpc.params.mem_alloc.region)
            {
...
mve_error mve_session_handle_rpc_mem_resize(struct mve_session *session, int fd)
{
    struct mve_com_rpc rpc;
    mve_error err;
    ...
    if (MVE_ERROR_NONE == err)
    {
        ...
        switch (rpc.call_id)
        {
            ...
            case MVE_COM_RPC_FUNCTION_DEBUG_PRINTF:
            {
                ...
                event = mve_queue_create_event(MVE_EVENT_RPC_PRINT, size);
                if (NULL == event)
                {
                    return;
                }
            ...
            case MVE_COM_RPC_FUNCTION_MEM_ALLOC:
            {
                ...
                uint32_t mve_addr = 0;
                bool secure;
                secure = mve_fw_secure(session->fw_inst);
                size = rpc.params.mem_alloc.size;
                max_size = rpc.params.mem_alloc.max_size;
                alignment = rpc.params.mem_alloc.log2_alignment;
                ...
                if (false != secure)
                {
                    ...
                    event = mve_queue_create_event(MVE_EVENT_RPC_MEM_ALLOC, sizeof(uint32_t));
                    *((uint32_t *)event->data) = num_pages * MVE_MMU_PAGE_SIZE;
                    mve_queue_add_event(session->mve_events, event);
                    MVE_RSRC_MEM_CACHE_FREE(event, EVENT_SIZE(event->size));
                    return;
                }
            ...
            case MVE_COM_RPC_FUNCTION_MEM_RESIZE:
            {
                ...
                if (false != secure)
                {
                    ...
                    if (num_pages > mapped_pages)
                    {
                        ...
                        event = mve_queue_create_event(MVE_EVENT_RPC_MEM_RESIZE, sizeof(uint32_t));
                        *((uint32_t *)event->data) = num_new_pages * MVE_MMU_PAGE_SIZE;
                        mve_queue_add_event(session->mve_events, event);
                        MVE_RSRC_MEM_CACHE_FREE(event, EVENT_SIZE(event->size));
                        return;
                    }
...
static void mve_session_hung(struct mve_session *session)
{
    ...
    event = mve_queue_create_event(MVE_EVENT_SESSION_HUNG, 0);
    if (NULL == event)
    {
        return;
    }
...
static void mve_session_switchin_callback(mver_session_id session_id)
{
    ...
    if (session->state.pending_hw_state != MVE_HW_STATE_PENDING &&
        session->state.hw_state != session->state.pending_hw_state)
    {
        uint32_t code = (session->state.pending_hw_state == MVE_HW_STATE_STOPPED) ? MVE_MESSAGE_CODE_STOP : MVE_MESSAGE_CODE_GO;
        ...
        session->state.pending_hw_state = MVE_HW_STATE_PENDING;
    }
...
struct mve_session *mve_session_create(struct file *filep)
{
    ...
    session->state.hw_state           = MVE_HW_STATE_STOPPED;
    ...
    session->state.eos_queued             = false;
    printk("mve_session_create  session->pending_response_count:%d\r\n",session->pending_response_count);
...
mve_error mve_session_enqueue_flush_buffers(struct mve_session *session, uint32_t flush)
{
    bool res;
    ...
    {
        ret = MVE_ERROR_HARDWARE;
        goto out;
    }

    if (0 == (flush & MVE_FLUSH_ALL_PORTS))
    {
        ret = MVE_ERROR_BAD_PARAMETER;
        goto out;
    }

    if (0 != (flush & MVE_FLUSH_INPUT_PORT))
    {
    ...
    if (0 != (flush & MVE_FLUSH_OUTPUT_PORT))
    {
    ...
    if (MVE_ERROR_NONE == ret && 0 != (flush & MVE_FLUSH_QUICK))
    {
        session->state.quick_flush_state = 0;
...
mve_error mve_session_enqueue_state_change(struct mve_session *session, enum mve_hw_state state)
{
    bool res;
    mve_error ret = MVE_ERROR_NONE;

    if (MVE_HW_STATE_STOPPED != state &&
        MVE_HW_STATE_RUNNING != state)
    {
        return MVE_ERROR_BAD_PARAMETER;
    }
...
struct mve_event_header *mve_session_get_event(struct mve_session *session, uint32_t timeout)
{
    struct mve_event_header *event = NULL;
    bool ret;
...
mve_error mve_session_set_paramconfig(struct mve_session *session,
                                      uint32_t size,
                                      void *data,
                                      ...
                                      enum MVE_ENTITY entity)
{
    ...
    if (WATCHDOG_TIMEOUT == session->state.watchdog_state)
    {
        printk("MVE_ERROR_HARDWARE 11111\r\n");
        ret = MVE_ERROR_HARDWARE;
        release_session(session);
        return ret;
    }
    ...
    printk("mve_session_set_paramconfig response_count:%d session->pending_response_count:%d ret:%d\r\n",response_count,session->pending_response_count,ret);
    ...
    if (MVE_ERROR_NONE == ret)
    {
        ...
        while (response_count-- > 0)
        {
            ...
            if (MVE_EVENT_SET_PARAMCONFIG != event->code ||
                sizeof(struct set_response) != event->size)
            {
                printk("MVE_ERROR_HARDWARE 222222\r\n");
                ret = MVE_ERROR_HARDWARE;
                MVE_RSRC_MEM_CACHE_FREE(event, EVENT_SIZE(event->size));
                goto out;
            }
            ...
            if (response->error != 0)
            {
                *firmware_error = response->error;                
                printk("MVE_ERROR_FIRMWARE 11111\r\n");
                ret = MVE_ERROR_FIRMWARE;
            }
    ...
    else
    {
        ...
        {
            *firmware_error = ret;            
            printk("MVE_ERROR_FIRMWARE 222222\r\n");
            ret = MVE_ERROR_FIRMWARE;
        }
        change_session_state(session, STATE_ACTIVE_JOB);
    }
...
mve_error mve_session_get_paramconfig(struct mve_session *session,
                                      struct mve_command_header *command,
                                      void *data,
                                      ...
                                      enum MVE_ENTITY entity)
{
    ...
    if (NULL == event)
    {
         ...
         * MVE_ERROR_NOT_READY is returned. */
        ret = MVE_ERROR_NOT_READY;
        change_session_state(session, STATE_ACTIVE_JOB);
        goto out2;
    }
    ...
    if (MVE_EVENT_GET_PARAMCONFIG != event->code || (command->size + sizeof(uint32_t)) != event->size)
    {
...
mve_error mve_session_buffer_register(struct mve_session *session,
                                      uint32_t port_index,
                                      struct mve_buffer_userspace *descriptor)
{
    mve_error ret;
    bool res;
    ...
    if (MVE_BUFFER_FORMAT_YUV420_AFBC == descriptor->format ||
        MVE_BUFFER_FORMAT_YUV420_AFBC_10B == descriptor->format)
    {
        uint32_t fuse = mver_reg_get_fuse();
        if (fuse & (1 << CORESCHED_FUSE_DISABLE_AFBC))
        {
            return MVE_ERROR_NOT_IMPLEMENTED;
        }
    }
...
mve_error mve_session_buffer_unregister(struct mve_session *session,
                                        mve_buffer_handle_t buffer_id)
{
    mve_error ret;
    bool res;
	int sem_taken;
...
mve_error mve_session_buffer_enqueue(struct mve_session *session,
                                     struct mve_buffer_details *param,
...
mve_error mve_session_buffer_notify_ref_frame_release(struct mve_session *session,
                                                      mve_buffer_handle_t buffer_id)
{
    mve_error ret = MVE_ERROR_NONE;
    bool res;
...
mve_error mve_session_enqueue_message(struct mve_session *session,
                                      uint32_t code,
                                      uint32_t size,
                                      void *data)
{
