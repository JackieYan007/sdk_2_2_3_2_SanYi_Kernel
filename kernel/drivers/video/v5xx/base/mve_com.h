...
struct mve_com_buffer_frame
{
    ...
    enum mve_buffer_format format; /* Buffer format. */
...
{
    uint8_t nRegions;
    struct roi_region regions[MAX_ROI_REGIONS];
};
...
union mve_com_rpc_params
{
    ...
    struct
    {
        ...
        uint8_t region; /* Memory region selection */
            #define MVE_COM_MEM_REGION_PROTECTED (0)
            #define MVE_COM_MEM_REGION_OUTBUF    (1)
            #define MVE_COM_MEM_REGION_FRAMEBUF  (MVE_COM_MEM_REGION_OUTBUF)
...
{
    mve_error (*add_message)(struct mve_session *session, uint16_t code, uint16_t size, uint32_t *data);
    uint32_t *(*get_message)(struct mve_session *session, struct mve_msg_header *header);
    mve_error (*add_input_buffer)(struct mve_session *session, mve_com_buffer *buffer, enum mve_com_buffer_type type);
    mve_error (*add_output_buffer)(struct mve_session *session, mve_com_buffer *buffer, enum mve_com_buffer_type type);
    mve_error (*get_input_buffer)(struct mve_session *session, mve_com_buffer *buffer);
    mve_error (*get_output_buffer)(struct mve_session *session, mve_com_buffer *buffer);
    mve_error (*get_rpc_message)(struct mve_session *session, mve_com_rpc *rpc);
    mve_error (*put_rpc_message)(struct mve_session *session, mve_com_rpc *rpc);
};
 ...
 * @return MVE_ERROR_NONE or success, else error code.
 */
 ...
 * @return MVE_ERROR_NONE if successful, otherwise a suitable error code.
 */
 ...
 * @return MVE_ERROR_NONE if the buffer was added successfully. Error code otherwise.
 */
 ...
 * @return MVE_ERROR_NONE if buffer contains a valid buffer. Error code otherwise.
 */
 ...
 * @return MVE_ERROR_NONE if rpc contains valid data. Error code otherwise.
 */
