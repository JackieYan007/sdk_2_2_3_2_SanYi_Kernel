                                    ...
                                    struct mve_buffer_external *buffer)
{
    if (NULL != buffer)
    {
        if (MVE_BUFFER_ALLOCATOR_ATTACHMENT != buffer->info.allocator)
        {
                                                         ...
                                                         struct mve_buffer_info *info)
{
    ...
    else
    {
        ...
        if (MVE_BUFFER_ALLOCATOR_ATTACHMENT != info->allocator)
        {
...
mve_error mve_session_buffer_register_internal(struct mve_session *session,
                                               uint32_t port_index,
                                               struct mve_buffer_userspace *userspace_buffer)
{
    ...
    crc_info.format    = MVE_BUFFER_FORMAT_CRC;
    crc_info.offset    = userspace_buffer->crc_offset;
    crc_info.do_cache_maintenance = true;
    if ((userspace_buffer->mve_flags & MVE_FLAGS_DMABUF_DISABLE_CACHE_MAINTENANCE) != 0 &&
        MVE_BUFFER_ALLOCATOR_DMABUF == userspace_buffer->allocator)
    {
    ...
    if (0 != crc_info.handle && MVE_BUFFER_ALLOCATOR_ATTACHMENT == crc_info.allocator &&
...
mve_error mve_session_buffer_unregister_internal(struct mve_session *session,
                                                 mve_buffer_handle_t buffer_id)
{
    struct mve_buffer_client *buffer;
    mve_error ret = MVE_ERROR_NONE;
...
mve_error mve_session_buffer_enqueue_internal(struct mve_session *session,
                                              ...
                                              bool empty_this_buffer)
{
    ...
    {
        case MVE_BUFFER_FORMAT_BITSTREAM:
        ...
        case MVE_BUFFER_FORMAT_YV12:
            ...
            if (MVE_BUFFER_FORMAT_YV12 == buffer_info->format)
            {
            ...
            else /* if (MVE_BUFFER_FORMAT_YUV420_PLANAR == buffer_info->format) */
            {
        ...
        case MVE_BUFFER_FORMAT_YUYYVY_10B:
            ...
            break;
        case MVE_BUFFER_FORMAT_YUV420_AFBC: /* Intentional fallthrough */
        ...
        0 != param->nRegions)
    {
        ...
        WARN_ON(ret != MVE_ERROR_NONE);
    }

    /* Setup CRC buffer and correct MVE flags */
    switch (buffer_info->format)
    {
        case MVE_BUFFER_FORMAT_YUV420_PLANAR: /* Intentional fallthrough */
        case MVE_BUFFER_FORMAT_YUV420_SEMIPLANAR:
        case MVE_BUFFER_FORMAT_YVU420_SEMIPLANAR:
        case MVE_BUFFER_FORMAT_YUYYVY_10B:
        ...
        case MVE_BUFFER_FORMAT_ABGR_8888:
            ...
            if (NULL != buffer_client->crc)
            {
                ...
                {
                    if (MVE_BUFFER_ALLOCATOR_ATTACHMENT == buffer_client->crc->info.allocator)
                    {
...
mve_error mve_session_buffer_dequeue_internal(struct mve_session *session,
                                              mve_buffer_handle_t buffer_id,
                                              mve_com_buffer *buffer)
{
    ...
    if (MVE_BUFFER_FORMAT_BITSTREAM == buffer_info->format)
    {
        ...
        buffer_client->timestamp  = buffer->bitstream.timestamp;
             ...
             MVE_BUFFER_FORMAT_YUV420_SEMIPLANAR == buffer_info->format ||
             MVE_BUFFER_FORMAT_YV12 == buffer_info->format ||
             MVE_BUFFER_FORMAT_YVU420_SEMIPLANAR == buffer_info->format)
    {
    ...
    else if (MVE_BUFFER_FORMAT_YUYYVY_10B == buffer_info->format ||
             MVE_BUFFER_FORMAT_YUV422_1P == buffer_info->format)
    {
        if (buffer->frame.nMVEFlags & (MVE_FLAGS_TOP_PRESENT | MVE_FLAGS_BOT_PRESENT))
        {
    ...
    else if (MVE_BUFFER_FORMAT_YUV420_AFBC == buffer_info->format ||
             MVE_BUFFER_FORMAT_YUV420_AFBC_10B == buffer_info->format)
    {
        if (buffer->frame.nMVEFlags & (MVE_FLAGS_TOP_PRESENT | MVE_FLAGS_BOT_PRESENT))
        {
            if (0 == buffer_info->afbc_alloc_bytes)
            {
                ...
                if (MVE_BUFFER_FORMAT_YUV420_AFBC == buffer_info->format)
                {
...
mve_error mve_session_buffer_notify_ref_frame_release_internal(struct mve_session *session,
                                                               mve_buffer_handle_t buffer_id)
{
    struct mve_buffer_client *buffer_client;
...
void mve_session_buffer_convert_to_userspace(struct mve_buffer_userspace *dst,
                                             struct mve_buffer_client *src)
{
    memset(dst, 0, sizeof(struct mve_buffer_userspace));
