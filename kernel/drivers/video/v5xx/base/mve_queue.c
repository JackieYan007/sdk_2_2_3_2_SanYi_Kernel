 ...
 */
static struct mve_event_header get_event_header(struct mve_queue *queue)
{
    uint32_t buf_size = queue->buffer_size;

    return *((struct mve_event_header *)&queue->buffer[queue->rpos % buf_size]);
}

struct mve_event_header *mve_queue_wait_for_event(struct mve_queue *queue, uint32_t timeout)
{
    struct mve_event_header *event = NULL;
    struct mve_event_header header;
    uint32_t buf_size;
    uint32_t words;
    uint32_t i;
    uint32_t *dst;
    ...
    for (i = 0; i < words; ++i)
    {
        dst[i] = queue->buffer[(sizeof(struct mve_event_header) / sizeof(uint32_t) +
                                queue->rpos + i) % buf_size];
    }

out:
    queue->rpos += (sizeof(struct mve_event_header) + words * sizeof(uint32_t)) /
                   sizeof(uint32_t);
exit:
    if (0 == sem_taken)
    {
                         ...
                         struct mve_event_header *event)
{
    ...
    buf_size = queue->buffer_size;
    if (queue->wpos + sizeof(struct mve_event_header) / sizeof(uint32_t) + words -
        queue->rpos > buf_size)
    {
        /* Drop this event! */
        /* WARN_ON(true); */
    ...
    header = (struct mve_event_header *)(dst + (queue->wpos++ % buf_size));
...
struct mve_event_header *mve_queue_create_event(int code, int size)
{
    struct mve_event_header *event;
