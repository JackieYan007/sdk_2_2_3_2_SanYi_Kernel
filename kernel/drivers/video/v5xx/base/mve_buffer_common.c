...
bool mve_buffer_is_valid(const struct mve_buffer_info *info)
{
    ...
    if (MVE_BUFFER_FORMAT_BITSTREAM != info->format &&
        MVE_BUFFER_FORMAT_CRC != info->format)
    {
        if (0 == info->width ||
            0 == info->height ||
            0 == info->stride)
    ...
    if (MVE_BUFFER_ALLOCATOR_VMALLOC != info->allocator &&
        MVE_BUFFER_ALLOCATOR_DMABUF != info->allocator &&
        MVE_BUFFER_ALLOCATOR_ASHMEM != info->allocator &&
        MVE_BUFFER_ALLOCATOR_ATTACHMENT != info->allocator)
    {
        return false;
    }
    ...
    switch (info->format)
    {
        ...
        case MVE_BUFFER_FORMAT_YV12:
            ...
            if (MVE_BUFFER_ALLOCATOR_DMABUF == info->allocator ||
                MVE_BUFFER_ALLOCATOR_ASHMEM == info->allocator)
            {
