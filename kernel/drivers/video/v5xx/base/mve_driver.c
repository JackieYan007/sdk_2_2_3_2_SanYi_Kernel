...
static long mve_driver_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    switch (cmd)
    {
        case MVE_COMMAND:
        {
            struct mve_command_header header;
            void *data = NULL;
            struct mve_response *result;
            struct mve_response_header tmp;
            struct mve_response_header *dst;

            /* Get command header */
            if (0 != copy_from_user(&header, (void __user *)arg, sizeof(struct mve_command_header)))
            {
            ...
            if (0 < header.size)
            {
                ...
                if (0 != copy_from_user(data,
                                        (void __user *)(arg + sizeof(struct mve_command_header)),
                                        header.size))
                {
            ...
            dst = (struct mve_response_header *)arg;
...
static int mve_driver_open(struct inode *inode, struct file *file)
{
    ...
    if (0 != ret)
    {
        MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_ERROR, "Unable to register interrupt handler: \'%d\'", ret);
    }
...
static int mve_driver_probe(struct platform_device *pdev)
{
    ...
    printk("MVE base0704 driver loaded successfully\n");
