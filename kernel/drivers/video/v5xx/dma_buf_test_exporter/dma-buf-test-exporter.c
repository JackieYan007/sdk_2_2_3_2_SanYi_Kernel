...
static int dma_buf_te_mmap(struct dma_buf *dmabuf, struct vm_area_struct *vma)
{
	...
	dev_info(te_device.this_device, "%s dma_addr %x pages %d\n", __func__,alloc->contig_dma_addr,alloc->nr_pages);

	/*  we fault in the pages on access */
#if 1 
	int ret = remap_pfn_range(vma, vma->vm_start, PFN_DOWN(alloc->contig_dma_addr), alloc->nr_pages * PAGE_SIZE,
				  vma->vm_page_prot);

	dev_info(te_device.this_device, "%s %x ret = %d\n", __func__,vma->vm_start,ret);
	if (ret)
	{
		return ret;
	}
