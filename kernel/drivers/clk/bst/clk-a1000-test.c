...
static const struct of_device_id of_clktest_match[] = {
	{
		.compatible = "bst,a1000-clkc-test"
	},
	{ },
};
...
static int clktest_probe(struct platform_device *pdev)
{
    ...
    pr_debug("This is %s:%d.\n", __func__, __LINE__);

    bst_clk_test_dev = &pdev->dev;    
	
	if (bst_clk_test_dev_major) {
		devid = MKDEV(bst_clk_test_dev_major, 0);
		register_chrdev_region(devid, CLKTEST_CNT, "bsta1000_clktest"); 
	} else {
		alloc_chrdev_region(&devid, 0, CLKTEST_CNT, "bsta1000_clktest");
	...
	cls = class_create(THIS_MODULE, "bsta1000_clktest");
    device_create(cls, NULL, MKDEV(bst_clk_test_dev_major, 0), NULL, "bsta1000_clktest"); /* /dev/bsta1000_clktest */

    pr_debug("This is %s:line %d.\n", __func__, __LINE__);
...
static struct platform_driver bsta1000_clktest_driver = {
	.probe		= clktest_probe,
	.remove		= clktest_remove,
	.driver		= {
		.name	= "bsta1000_clktest",
		.of_match_table	= of_match_ptr(of_clktest_match),
	},
};

module_platform_driver(bsta1000_clktest_driver);
