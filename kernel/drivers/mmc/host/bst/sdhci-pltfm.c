...
void sdhci_get_property(struct platform_device *pdev)
{
	...
	if (device_property_present(dev, "broken-adma"))
		host->quirks |= SDHCI_QUIRK_BROKEN_ADMA;

	if (device_property_present(dev, "broken-dma"))
		host->quirks |= SDHCI_QUIRK_BROKEN_DMA;

	if (device_property_present(dev, "broken-multiblk"))
		host->quirks |= SDHCI_QUIRK_NO_MULTIBLOCK;

	if (device_property_present(dev, "broken-64bit-dma"))
		host->quirks2 |= SDHCI_QUIRK2_BROKEN_64_BIT_DMA;

	if (device_property_present(dev, "clear-tarns-mode"))
		host->quirks2 |= SDHCI_QUIRK2_CLEAR_TRANSFERMODE_REG_BEFORE_CMD;

	if (device_property_present(dev, "hostoff-cardon"))
		host->quirks2 |= SDHCI_QUIRK2_HOST_OFF_CARD_ON;

	if (device_property_present(dev, "cardon-buson"))
	...
	if (device_property_present(dev, "no-3-3-v"))
		host->quirks2 |= SDHCI_QUIRK2_MMC_NO_3_3V;
