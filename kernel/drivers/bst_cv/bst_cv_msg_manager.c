 ...
 *          error code - failure
 */
int bst_cv_msg_recv(struct bst_cv *pbst_cv, int dsp, uint32_t *data, int timeout)
{
    int ret;
    ipc_msg msg;
...
static int bst_cv_worker(void *args)
{
    ...
    struct xrp_dsp_cmd *xrp_dsp_comm_base = pbst_cv->fw_manager.dsps[dsp].sync_virt_base;
    ...
    do {
        ...
        if (bst_cv_msg_send(pbst_cv, dsp, 0) < 0) {
            BST_CV_DEV_ERR(&pbst_cv->pdev->dev, "failed to send the request!");
            req->result = XRP_STATUS_FAILURE;
        }
        else {
wait_for_response:
            if (bst_cv_msg_recv(pbst_cv, dsp, &data, BST_CV_MSG_TIMEOUT) < 0) {
                BST_CV_DEV_ERR(&pbst_cv->pdev->dev, "ipc timeout, failed to receive the response!");
                req->result = XRP_STATUS_FAILURE;
            }
        ...
        if (req->queue.cookie == wait->cookie) {
            ...
            if (pbst_cv->state == BST_CV_ONLINE) {
                if (req->result == XRP_STATUS_SUCCESS) {
                    ...
                    if (ret < 0) {
                        BST_CV_DEV_ERR(&pbst_cv->pdev->dev, "failed to copy out_data!");
                    }
                }
                else {
                    ret = -EFAULT;
...
int bst_cv_msg_manager_init(struct bst_cv *pbst_cv)
{
    ...
    pbst_cv->msg_manager.zombie_list = (struct list_head)LIST_HEAD_INIT(pbst_cv->msg_manager.zombie_list);
    pbst_cv->msg_manager.cleaner = kthread_run(bst_cv_cleaner, pbst_cv, "bst_cv_cleaner");
    if (IS_ERR(pbst_cv->msg_manager.cleaner)) {
        BST_CV_DEV_ERR(&pbst_cv->pdev->dev, "kthread_run failed to create the cleaner thread!");
    ...
    for (i=0; i<BST_CV_DSP_NUM; i++) {
        if (pbst_cv->dsp_online[i]) {
            ...
            cur->ipc_session_id = ipc_init(IPC_CORE_DSP_0+i,
                IPC_CORE_ARM0+pbst_cv->fw_manager.dsps[i].ipc_src_core, &pbst_cv->pdev->dev);
            if (cur->ipc_session_id < 0) {
                pbst_cv->dsp_online[i] = 0;
                BST_CV_DEV_ERR(&pbst_cv->pdev->dev, "ipc_init failed for DSP %d, ret %d",
                    i, cur->ipc_session_id);
            }
            if (ipc_signal_subscribe(cur->ipc_session_id, 0) != 0) {
                pbst_cv->dsp_online[i] = 0;
                BST_CV_DEV_ERR(&pbst_cv->pdev->dev,"session %d subscribe failed", cur->ipc_session_id);
            }
        }
    }
    if (!bst_cv_check_online(pbst_cv)) {
        BST_CV_DEV_ERR(&pbst_cv->pdev->dev, "ipc_init failed for all DSPs");
        return -EFAULT;
    }
    ...
    for (i=0; i<BST_CV_DSP_NUM; i++) {
        if (pbst_cv->dsp_online[i]) {
            ...
            cur->work_list = (struct list_head)LIST_HEAD_INIT(cur->work_list);
            spin_lock_init(&cur->wl_lock);

            worker_name[6] = '0' + i;
            worker_name[7] = '\0';
            cur->worker = kthread_run(bst_cv_worker, &pbst_cv->msg_manager.dsps[i], worker_name);
            if (IS_ERR(cur->worker)) {
                pbst_cv->dsp_online[i] = 0;
                ipc_close(cur->ipc_session_id);
                BST_CV_DEV_ERR(&pbst_cv->pdev->dev, "kthread_run failed for DSP %d!", i); 
                continue;
            }
    ...
    if (!bst_cv_check_online(pbst_cv)) {
        BST_CV_DEV_ERR(&pbst_cv->pdev->dev, "kthread_run failed for all DSPs");
        return -EFAULT;
    }
    else {
