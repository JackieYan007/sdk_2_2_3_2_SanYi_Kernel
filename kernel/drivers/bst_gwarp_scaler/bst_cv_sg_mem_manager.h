...
#ifndef BST_CV_GS_MEM_MANAGER_H
#define BST_CV_GS_MEM_MANAGER_H
...
#define kern_to_bus(pbst_cv, vaddr) phys_to_bus(pbst_cv, kern_to_phys(pbst_cv, vaddr))
...
struct bst_cv_mem_manager {
    ...
    phys_addr_t reserved_phy_addr;
    __u32 reserved_phy_addr_len;
...
int bst_get_user_addr_by_bus(struct file *filp, struct bst_cv *pbst_cv, phys_addr_t phy_addr, __u64 *user_addr);
...
int bst_cv_ioctl_dma_buf_import(struct bst_cv *pbst_cv, buf_req_t __user *ubuf_req);
