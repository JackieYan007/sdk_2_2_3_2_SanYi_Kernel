...
static int check_edid(unsigned char *edid)
{
	...
	if (ret)
		pr_info("fbmon: The EDID Block of Manufacturer: %s Model: 0x%x is known to be broken\n",
			manufacturer, model);
...
static void fix_edid(unsigned char *edid, int fix)
{
	...
	case FBMON_FIX_INPUT:
		...
		break;
	case FBMON_FIX_TIMINGS:
		pr_info("fbmon: trying to fix monitor timings\n");
		b = edid + DETAILED_TIMING_DESCRIPTIONS_START;
		for (i = 0; i < 4; i++) {
...
static void parse_vendor_block(unsigned char *block, struct fb_monspecs *specs)
{
	...
	specs->serial = block[4] + (block[5] << 8) + (block[6] << 16) +
			(block[7] << 24);
			      ...
			      struct fb_videomode *mode)
{
	...
	if (var) {
		var->xres = xres;
		var->yres = yres;
		fb_get_mode(FB_VSYNCTIMINGS | FB_IGNOREMON, refresh, var, NULL);
		mode->xres = xres;
		mode->yres = yres;
			  ...
			  int ver, int rev, const struct fb_monspecs *specs)
{
	...
	if (i < DMT_SIZE && dmt_modes[i].mode) {
		...
		*mode = *dmt_modes[i].mode;
		...
		switch (ratio) {
		case 0:
			/* in EDID 1.3 the meaning of 0 changed to 16:10 (prior
			 * 1:1)
			 */
...
static void get_detailed_timing(unsigned char *block, struct fb_videomode *mode)
{
	...
	mode->refresh = PIXEL_CLOCK /
			((H_ACTIVE + H_BLANKING) * (V_ACTIVE + V_BLANKING));
	if (INTERLACED) {
		mode->yres *= 2;
		mode->upper_margin *= 2;
		mode->lower_margin *= 2;
...
static int fb_get_monitor_limits(unsigned char *edid, struct fb_monspecs *specs)
{
	...
	if (retval) {
		...
		for (i = 0; i < num_modes; i++) {
			...
			htotal = mode->xres + mode->right_margin +
				 mode->hsync_len + mode->left_margin;
			vtotal = mode->yres + mode->lower_margin +
	...
	for (i = 0; i < 4; i++, block += DETAILED_TIMING_DESCRIPTION_SIZE) {
		if (edid_is_timing_block(block)) {
			...
			var->left_margin =
				...
				(H_ACTIVE + H_SYNC_OFFSET + H_SYNC_WIDTH);
			var->upper_margin =
				V_BLANKING - V_SYNC_OFFSET - V_SYNC_WIDTH;
...
static void fb_timings_vfreq(struct __fb_timings *timings)
{
	...
	timings->hblank =
		fb_get_hblank_by_hfreq(timings->hfreq, timings->hactive);
...
{
	timings->hblank =
		fb_get_hblank_by_dclk(timings->dclk, timings->hactive);
	timings->htotal = timings->hactive + timings->hblank;
		...
		struct fb_info *info)
{
	...
	if (err || (!(flags & FB_IGNOREMON) &&
		...
		var->left_margin =
			timings->hblank - var->right_margin - var->hsync_len;
...
{
	pr_debug(
		"fb_videomode = %ux%u@%uHz (%ukHz) %u %u %u %u %u %u %u %u %u\n",
...
int fb_validate_mode(const struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	return (vfreq < vfmin || vfreq > vfmax || hfreq < hfmin ||
		hfreq > hfmax || pixclock < dclkmin || pixclock > dclkmax) ?
		       -EINVAL :
		       0;
}
