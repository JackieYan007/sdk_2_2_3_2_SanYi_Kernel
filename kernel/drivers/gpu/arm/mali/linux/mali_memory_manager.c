...
_mali_osk_errcode_t _mali_ukk_mem_allocate(_mali_uk_alloc_mem_s *args)
{
	...
	MALI_DEBUG_PRINT(4, (" _mali_ukk_mem_allocate, vaddr=0x%x, size =0x%x vsize=0x%x  flags=0x%x! \n", args->gpu_vaddr, args->psize,args->vsize,args->flags));
	...
	*/
	...
	if (args->flags & _MALI_MEMORY_ALLOCATE_SWAPPABLE) {
        MALI_DEBUG_PRINT(4, (" _MALI_MEMORY_ALLOCATE_SWAPPABLE \n"));
		mali_allocation->type = MALI_MEM_SWAP;
	} else if (args->flags & _MALI_MEMORY_ALLOCATE_RESIZEABLE) {
        MALI_DEBUG_PRINT(4, (" _MALI_MEMORY_ALLOCATE_RESIZEABLE \n"));
        ...
        MALI_DEBUG_PRINT(4, (" _MALI_MEMORY_ALLOCATE_SECURE \n"));
		mali_allocation->type = MALI_MEM_SECURE;
	} else if (MALI_TRUE == mali_memory_have_dedicated_memory()) {
        MALI_DEBUG_PRINT(4, (" mali_memory_have_dedicated_memory \n"));
		mali_allocation->type = MALI_MEM_BLOCK;
	} else {
        MALI_DEBUG_PRINT(4, (" MALI_MEM_OS \n"));
		mali_allocation->type = MALI_MEM_OS;
	}
			...
			*/
			if (mem_backend->type == MALI_MEM_OS) {
				...
				if (mali_mem_block_alloc(&mem_backend->block_mem, mem_backend->size)) {
					...
					mali_allocation->type = MALI_MEM_OS;                    
                    MALI_DEBUG_PRINT(4, (" MALI_MEM_OS2222 \n"));
				}
	...
	*/
    ...
    MALI_DEBUG_PRINT(4, (" _mali_ukk_mem_allocate ,mem_backend->type=0x%x\n",mem_backend->type));
	return _MALI_OSK_ERR_OK;
