...
static struct resource mali_gpu_resources_m450_mp3[] = {
    MALI_GPU_RESOURCES_MALI450_MP3_PMU(0x33300000, 165, 166, 159, 160,161, 162,163, 164,168)
};

#endif

static struct resource mali_gpu_resources_m450_mp3[] = {
    MALI_GPU_RESOURCES_MALI450_MP3_PMU(0x33300000, 132, 133, 127, 128,129, 130,131, 132,136)
};
...
static struct mali_gpu_device_data mali_gpu_data = {
	...
	.pp_cores =3,/* 3 pp cores for mali gpu */

	.dedicated_mem_start = 0xe0000000, /* Physical start address (use 0xD0000000 for old indirect setup) */
	.dedicated_mem_size = 0x2500000, /* 64MB */
...
int mali_platform_device_register(void)
{
    ...
    num_pp_cores = 3;// 1 pp only for now
    mali_gpu_device.num_resources = ARRAY_SIZE(mali_gpu_resources_m450_mp3);
    mali_gpu_device.resource = mali_gpu_resources_m450_mp3;
    m400_gp_version = mali_read_phys(0x3330006C);
    MALI_DEBUG_PRINT(4, ("Registering Mali-450 MP3 device version:0x%x\n",m400_gp_version));
...
void mali_get_of_property(struct platform_device *device)
{
	struct device_node *np = device->dev.of_node;
    
    /* get real mali gpu pp cores from dts */
	of_property_read_u32(np, "ppcores",(u32*) &mali_gpu_data.pp_cores);
    /* get real framebuffer start addr and size */
	of_property_read_u32(np, "fb_start", (u32*)&mali_gpu_data.fb_start);
	of_property_read_u32(np, "fb_size", (u32*)&mali_gpu_data.fb_size);
    /* get dedicated mem addr and size for gpu to allocate memory */
	of_property_read_u32(np, "dedicated_mem_start", (u32*)&mali_gpu_data.dedicated_mem_start);
	of_property_read_u32(np, "dedicated_mem_size", (u32*)&mali_gpu_data.dedicated_mem_size);
    
}
EXPORT_SYMBOL_GPL(mali_get_of_property);
...
#endif
    mali_get_of_property(device);

    m400_gp_version = mali_read_phys(0x3330006C);
    MALI_DEBUG_PRINT(4, ("Registering Mali-450 MP3 device version:0x%x\n",m400_gp_version));
    num_pp_cores = mali_gpu_data.pp_cores;// 1 pp only for now
