...
/*
  This function is to fix the kernel bug caused by compatibility of IEEE Std 802.3(current)
  to IEEE Std 802.3bw-2015 in definition of BMSR(Base Mode Status Register, offset:0x01).

  Dp83tc811, compliant to IEEE Std 802.3bw-2015, defines PHY capability as 100Base-TX full-Duplex
  in bit-14 of BMSR, while Linux-5.10.59, compliant to IEEE Std 802.3(current), defines
  PHY capability as 100Base-X full-Duplex which includes capabilities of 100Base-TX full-Duplex,
  100Base-T1 full-Duplex, 100Base-T4 full-Duplex and etc. in the corresponding bit(bit-14).
  100Base-X full-Duplex comtains the capability of 100Base-TX full-Duplex. To meet validation check
  of capabilities in BST-A1000B-GMAC driver, dp83811 driver should tell GMAC driver by kernel that
  dp83tc811 is capable of 100Base-X full-Duplex rather than incapable of 100Base-TX full-Duplex.

  BST-A1000B-GMAC driver in Linux-4.19.66 have not the validation check of capabilities.

  IEEE Std 802.3bw-2015(100BASE-T1), is amendment to IEEE Std 802.3-2015 which was superseded
  by IEEE Std 802.3(current). We do not recommend to use such hardware compliant to standards that
  was superseded.

  By Bob.Wang@BST.AI@17:20-2022-04-18
*/
static int dp83811_get_features(struct phy_device *phydev)
{
	int err;
...
static struct phy_driver dp83811_driver[] = {
	{
		...
		.get_features = dp83811_get_features,
	 },
};
