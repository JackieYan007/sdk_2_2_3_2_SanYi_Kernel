...
int bst_89x_miiphy_read(struct phy_device *phy, int devad, unsigned char reg,unsigned short *value)
{
	int ret;
 ...
 *   0 on success
 */
int bst_89x_miiphy_write(struct phy_device *phy, int devad, unsigned char reg, unsigned short value)
{
...
int bst_89x_waitfor_link(struct phy_device *phy)
{
	...
	int cnt = 0;
	printk("Waiting for PHY Link to complete");
...
{   
    ...
    bst_89x_miiphy_read(phy, BST_GMAC1_DEVAD, 0x00, &value);
	...
	bst_89x_miiphy_write(phy, BST_GMAC1_DEVAD, 0x0b, 0x6ff6);
...
static int bcm89881_reset(struct phy_device *phy)
{
    unsigned short value;

    bst_89x_miiphy_write(phy, BST_GMAC1_DEVAD, 0x0000, 0x8040);	 
	mdelay(1000);
    value = 0;
    bst_89x_miiphy_read(phy, BST_GMAC1_DEVAD, 0x0000, &value);
    if (value & 0x8000) {
        return -1;
    }
...
static int bcm89881_read_status(struct phy_device  *phy)
{
    unsigned short value;
...
static int bcm89881_config_aneg(struct phy_device *phy)
{
...
static int bcm89881_aneg_done(struct phy_device *phy)
{
    return 1;
}
...
static int bcm89881_config(struct phy_device *phydev)
{
    if (phydev->master_slave_get == MASTER_SLAVE_CFG_MASTER_FORCE) {
        printk("BCM PHY master init\n");
        return (bst_89x_master_init(phydev));
    } else if (phydev->master_slave_get == MASTER_SLAVE_CFG_SLAVE_FORCE) {
        printk("BCM PHY slave init\n");
        return (bst_89x_slave_init(phydev));
    } else {
        printk("BCM PHY role not init\n");
        return -1;
    }
}
...
void bcm89881_extend_op(struct phy_device *phydev)
{
    ...
    bst_89x_miiphy_write(phydev, 0x1, 0xa010, 0x0101);       
}
EXPORT_SYMBOL(bcm89881_extend_op);

int bst_bcm89830_init(struct phy_device *phy)
{   
    bst_89x_miiphy_write(phy, 7, 0x9028, 0x0c30); 
	bst_89x_miiphy_write(phy, 7, 0x912d, 0x9b52); 
    bst_89x_miiphy_write(phy, 7, 0x912e, 0xa04d);	 
	bst_89x_miiphy_write(phy, 7, 0x9125, 0x0331);	 
	bst_89x_miiphy_write(phy, 7, 0x9154, 0x88c4);	 
	bst_89x_miiphy_write(phy, 7, 0x9302, 0x9a65);	 
	bst_89x_miiphy_write(phy, 7, 0x91f3, 0x1000);	 
	bst_89x_miiphy_write(phy, 7, 0x917b, 0x00bf);
	bst_89x_miiphy_write(phy, 7, 0x917a, 0x00f1);
	bst_89x_miiphy_write(phy, 7, 0x9179, 0x0135);
	bst_89x_miiphy_write(phy, 7, 0x9178, 0x0185);
	bst_89x_miiphy_write(phy, 7, 0x9177, 0x01eb);
	bst_89x_miiphy_write(phy, 7, 0x9176, 0x0269);
	bst_89x_miiphy_write(phy, 7, 0x9175, 0x0309);
	bst_89x_miiphy_write(phy, 7, 0x9028, 0x0430);
    
	return bst_89x_waitfor_link(phy);
}

static void bst_bcm89830_master_force(struct phy_device *phy)
{
	bst_89x_miiphy_write(phy, 1, 0x0834, 0xc000);
}

static void bst_bcm89830_slave_force(struct phy_device *phy)
{
	bst_89x_miiphy_write(phy, 1, 0x0834, 0x8000);
}

static int bcm89830_reset(struct phy_device *phy)
{
    unsigned short value;

    bst_89x_miiphy_write(phy, 1, 0, 0xa000);	 
	udelay(10);
    value = 0;
    bst_89x_miiphy_read(phy, 1, 0, &value);
    if (value & 0x8000) {
        return -1;
    }
...
static int bcm89830_read_status(struct phy_device  *phy)
{
    unsigned short value;

    value = 0;
    bst_89x_miiphy_read(phy, 1, 1, &value);
    if (value & (1 << 2)) {
        phy->link = 1;
    } else {
        phy->link = 0;
...
static int bcm89830_config_aneg(struct phy_device *phy)
{
    phy->speed = SPEED_100;
    phy->duplex = DUPLEX_FULL;
...
static int bcm89830_aneg_done(struct phy_device *phy)
{
    return 1;
}

static int bcm89830_config(struct phy_device *phydev)
{
    if (phydev->master_slave_get == MASTER_SLAVE_CFG_MASTER_FORCE) {
        printk("BCM PHY master init\n");
        bst_bcm89830_master_force(phydev);
    } else if (phydev->master_slave_get == MASTER_SLAVE_CFG_SLAVE_FORCE) {
        printk("BCM PHY slave init\n");
        bst_bcm89830_slave_force(phydev);
    } else {
        printk("BCM PHY role not init\n");
        return -1;
    }

	bst_bcm89830_init(phydev);
	return 0;
}
...
static struct phy_driver broadcom_drivers[] = {
{
	...
	.phy_id         = PHY_ID_BCM8988x,
	.phy_id_mask    = 0xfffffffc,
	.name           = "Broadcom BCM8988x",
	/* PHY_GBIT_FEATURES */
	.config_init    = bcm89881_config,
	.ack_interrupt  = bcm_phy_ack_intr,
	.config_intr    = bcm_phy_config_intr,
    .soft_reset     = bcm89881_reset,
    .read_status	= bcm89881_read_status,
    .config_aneg    = bcm89881_config_aneg,
    .aneg_done      = bcm89881_aneg_done,
}, {
	.phy_id         = PHY_ID_BCM8983x,
	.phy_id_mask    = 0xfffffff0,
	.name           = "Broadcom BCM89830",
	/* PHY_GBIT_FEATURES */
	.config_init    = bcm89830_config,
	.ack_interrupt  = bcm_phy_ack_intr,
	.config_intr    = bcm_phy_config_intr,
    .soft_reset     = bcm89830_reset,
    .read_status	= bcm89830_read_status,
    .config_aneg    = bcm89830_config_aneg,
    .aneg_done      = bcm89830_aneg_done,
} };
...
static struct mdio_device_id __maybe_unused broadcom_tbl[] = {
    ...
    { PHY_ID_BCM8988x, 0xfffffffc },
	{ PHY_ID_BCM8983x, 0xfffffff0 },
	{ }
};
