...
#define MII_88E1510_GEN_CTRL_REG_1_MODE_RGMIITOSGMII	0x4	/* RGMII to SGMII */
...
#define MRVL_88Q2122_INIT_SLAVER        0x0000
#define MRVL_88Q2122_INIT_MASTER        0x0001
#define MRVL_88Q2122_INIT_AN_ENABLE     0x0002

#define MRVL_Q212X_LPSD_FEATURE_ENABLE  0

#define MARVELL_PHY_ID_88E6352      0x3520
#define MARVELL_88E6352_MASK        0xfff0
...
int bst_2122_miiphy_write(struct phy_device *phy, int devad, u32 reg, unsigned short value)
{
    return phy_write_mmd(phy, devad, reg, value);
}

int bst_2122_miiphy_read(struct phy_device *phy, int devad, u32 reg)
{
	int ret;
    
    ret = phy_read_mmd(phy, devad, reg);
    if (ret < 0) {
        return -1;
    }
...
void _applyQ2122GeSoftReset(struct phy_device *phydev)
{
    uint16_t regDataAuto = 0;

 	regDataAuto = bst_2122_miiphy_read(phydev, 1, 0x0000);
	regDataAuto |= 1 << 11;
	bst_2122_miiphy_write(phydev, 1, 0x0000, regDataAuto);
	bst_2122_miiphy_write(phydev, 3, 0xFFE4, 0x000C);
	msleep(10);
	bst_2122_miiphy_write(phydev, 3, 0xFFE4, 0x06B6);
	regDataAuto &= ~(1 << 11);
	bst_2122_miiphy_write(phydev, 1, 0x0000, regDataAuto);
	msleep(10);
	bst_2122_miiphy_write(phydev, 3, 0xFC47, 0x0030);
	bst_2122_miiphy_write(phydev, 3, 0xFC47, 0x0031);
	bst_2122_miiphy_write(phydev, 3, 0xFC47, 0x0030);
	bst_2122_miiphy_write(phydev, 3, 0xFC47, 0x0000);
	bst_2122_miiphy_write(phydev, 3, 0xFC47, 0x0001);
	bst_2122_miiphy_write(phydev, 3, 0xFC47, 0x0000);
	bst_2122_miiphy_write(phydev, 3, 0x0900, 0x8000);
	bst_2122_miiphy_write(phydev, 1, 0x0900, 0x0000);
	bst_2122_miiphy_write(phydev, 3, 0xFFE4, 0x000C);
}
...
uint16_t getMasterSlave (struct phy_device *phydev) {
    return ( (bst_2122_miiphy_read(phydev, 7, 0x8001) >> 14) & 0x0001 );
}
...
bool checkLink(struct phy_device *phydev)
{
	...
	bst_2122_miiphy_read(phydev, 3, 0x0901);
	retData1 = bst_2122_miiphy_read(phydev, 3, 0x0901);
	retData2 = bst_2122_miiphy_read(phydev, 7, 0x8001);
	msleep(5);
...
void initQ2122Ge (struct phy_device *phydev)
{
    uint16_t regData = 0;

	msleep(20);
	bst_2122_miiphy_write(phydev, 1, 0x0900, 0x4000);		
	bst_2122_miiphy_write(phydev, 7, 0x0200, 0x0000);		
	regData = bst_2122_miiphy_read(phydev, 1, 0x0834);
	regData = (regData & 0xFFF0) | 0x0001;
	bst_2122_miiphy_write(phydev, 1, 0x0834, regData);		
	bst_2122_miiphy_write(phydev, 3, 0xFFE4, 0x07B5);		
	bst_2122_miiphy_write(phydev, 3, 0xFFE4, 0x06B6);		
	msleep(50);
	bst_2122_miiphy_write(phydev, 3, 0xFFDE, 0x402F);		
	bst_2122_miiphy_write(phydev, 3, 0xFE2A, 0x3C3D);		
	bst_2122_miiphy_write(phydev, 3, 0xFE34, 0x4040);		
	bst_2122_miiphy_write(phydev, 3, 0xFE4B, 0x9337);		
	bst_2122_miiphy_write(phydev, 3, 0xFE2A, 0x3C1D);		
	bst_2122_miiphy_write(phydev, 3, 0xFE34, 0x0040);		
	bst_2122_miiphy_write(phydev, 3, 0xFE0F, 0x0000);		
	bst_2122_miiphy_write(phydev, 3, 0xFC00, 0x01C0);		
	bst_2122_miiphy_write(phydev, 3, 0xFC17, 0x0425);		
	bst_2122_miiphy_write(phydev, 3, 0xFC94, 0x5470);		
	bst_2122_miiphy_write(phydev, 3, 0xFC95, 0x0055);
	bst_2122_miiphy_write(phydev, 3, 0xFC19, 0x08d8);		
	bst_2122_miiphy_write(phydev, 3, 0xFC1A, 0x0110);
	bst_2122_miiphy_write(phydev, 3, 0xFC1B, 0x0a10);
	bst_2122_miiphy_write(phydev, 3, 0xFC3A, 0x2725);		
	bst_2122_miiphy_write(phydev, 3, 0xFC61, 0x2627);		
	bst_2122_miiphy_write(phydev, 3, 0xFC3B, 0x1612);		
	bst_2122_miiphy_write(phydev, 3, 0xFC62, 0x1C12);		
	bst_2122_miiphy_write(phydev, 3, 0xFC9D, 0x6367);		
	bst_2122_miiphy_write(phydev, 3, 0xFC9E, 0x8060);		
	bst_2122_miiphy_write(phydev, 3, 0xFC00, 0x01C8);		
	bst_2122_miiphy_write(phydev, 3, 0x8000, 0x0000);		
	bst_2122_miiphy_write(phydev, 3, 0x8016, 0x0011);
	bst_2122_miiphy_write(phydev, 3, 0xFDA3, 0x1800);		
	bst_2122_miiphy_write(phydev, 3, 0xFE02, 0x00C0);		
	bst_2122_miiphy_write(phydev, 3, 0xFFDB, 0x0010);		
	bst_2122_miiphy_write(phydev, 3, 0xFFF3, 0x0020);		
	bst_2122_miiphy_write(phydev, 3, 0xFE40, 0x00A6);		
	bst_2122_miiphy_write(phydev, 3, 0xFE60, 0x0000);		
	bst_2122_miiphy_write(phydev, 3, 0xFE2A, 0x3C3D);		
	bst_2122_miiphy_write(phydev, 3, 0xFE4B, 0x9334);		
	bst_2122_miiphy_write(phydev, 3, 0xFC10, 0xF600);		
	bst_2122_miiphy_write(phydev, 3, 0xFC11, 0x073D);
	bst_2122_miiphy_write(phydev, 3, 0xFC12, 0x000D);
	bst_2122_miiphy_write(phydev, 3, 0xFC13, 0x0010);
    
	//LPSD feature
	if (MRVL_Q212X_LPSD_FEATURE_ENABLE) {
		if (getMasterSlave(phydev)){
			bst_2122_miiphy_write(phydev, 7, 0x8032, 0x005A);			
		} else {
			bst_2122_miiphy_write(phydev, 7, 0x8032, 0x0064);			
		}
		bst_2122_miiphy_write(phydev, 7, 0x8031, 0x0A01);
		bst_2122_miiphy_write(phydev, 7, 0x8031, 0x0C01);
		bst_2122_miiphy_write(phydev, 3, 0x800C, 0x0008);		
		bst_2122_miiphy_write(phydev, 7, 0x8032, 0x0001);		
		bst_2122_miiphy_write(phydev, 7, 0x8031, 0x0A1B);
		bst_2122_miiphy_write(phydev, 7, 0x8031, 0x0C1B);
		bst_2122_miiphy_write(phydev, 7, 0x8032, 0x000B);		
		bst_2122_miiphy_write(phydev, 7, 0x8031, 0x0A1C);
		bst_2122_miiphy_write(phydev, 7, 0x8031, 0x0C1C);
		bst_2122_miiphy_write(phydev, 3, 0xFE04, 0x0016);	
	} else{
		bst_2122_miiphy_write(phydev, 7, 0x8032, 0x0064);
		bst_2122_miiphy_write(phydev, 7, 0x8031, 0x0A01);
		bst_2122_miiphy_write(phydev, 7, 0x8031, 0x0C01);
		bst_2122_miiphy_write(phydev, 3, 0x800C, 0x0000);		
		bst_2122_miiphy_write(phydev, 7, 0x8032, 0x0002);	
		bst_2122_miiphy_write(phydev, 7, 0x8031, 0x0A1B);
		bst_2122_miiphy_write(phydev, 7, 0x8031, 0x0C1B);
		bst_2122_miiphy_write(phydev, 7, 0x8032, 0x0003);
		bst_2122_miiphy_write(phydev, 7, 0x8031, 0x0A1C);
		bst_2122_miiphy_write(phydev, 7, 0x8031, 0x0C1C);
		bst_2122_miiphy_write(phydev, 3, 0xFE04, 0x0008);		
	}
	_applyQ2122GeSoftReset(phydev);
}
...
{
    if (phydev->master_slave_get == MASTER_SLAVE_CFG_MASTER_FORCE) {
        printk("Mav PHY master init\n");
        setMasterSlave(phydev, MRVL_88Q2122_INIT_MASTER);
    } else if (phydev->master_slave_get == MASTER_SLAVE_CFG_SLAVE_FORCE) {
        printk("Mav PHY slave init\n");
        setMasterSlave(phydev, MRVL_88Q2122_INIT_SLAVER);
    } else {
        printk("Mav PHY role not init\n");
        return -1;
    }
...
static int  m88q2122_read_status (struct phy_device *phydev) 
{
	...
	bst_2122_miiphy_read(phydev, 3, 0x0901);
	retData = bst_2122_miiphy_read(phydev, 3, 0x0901); /* read again for reserve bug */
...
static int m88q2122_aneg_done(struct phy_device *phy)
{
    return 1;
}

static int m88e6352_config_init (struct phy_device *phydev) 
{
    phy_write(phydev, 1, 0xc03e); //link/speed1000/full duplex
    return 0;
}

static int m88e6352_config_aneg ( struct phy_device *phydev)
{
    phydev->speed = SPEED_1000;
    phydev->duplex = DUPLEX_FULL;
...
static int  m88e6352_read_status (struct phy_device *phydev) 
{
	uint16_t retData = 0;

	
	retData = phy_read(phydev, 0);
...
static int m88e6352_aneg_done(struct phy_device *phydev)
{
    return 1;
}

static int m88e6352_resume(struct phy_device *phydev)
{
    u16 data;

    data = phy_read(phydev, 4);
    data |= 3;
    phy_write(phydev, 4, data);

    return 0;
}

static int m88e6352_suspend(struct phy_device *phydev)
{
    u16 data;
...
static int m88e1121_config_aneg_rgmii_delays(struct phy_device *phydev)
{
	...
	if (phydev->interface == PHY_INTERFACE_MODE_RGMII || phydev->interface == PHY_INTERFACE_MODE_RGMII_ID)
...
static int m88e1512_rgmii_to_sgmii(struct phy_device *phydev)
{
    int err;
    ...
    /* In reg 20, write MODE[2:0] = 0x1 (RGMII to SGMII) */
    err = phy_modify(phydev, MII_88E1510_GEN_CTRL_REG_1,
                MII_88E1510_GEN_CTRL_REG_1_MODE_MASK,
                MII_88E1510_GEN_CTRL_REG_1_MODE_RGMIITOSGMII);
    if (err < 0)
        return err;
...
static int m88e1510_config_init(struct phy_device *phydev)
{
	...
	if (phydev->interface == PHY_INTERFACE_MODE_SGMII) {
        ...
        m88e1512_rgmii_to_sgmii(phydev);
...
static int m88q2122_hwmon_probe(struct phy_device *phydev)
{
	return marvell_hwmon_probe(phydev, &m88e6390_hwmon_chip_info);
}
#else
...
static int m88q2122_probe(struct phy_device *phydev)
{
	...
	return m88q2122_hwmon_probe(phydev);
}

static int m88e6352_probe(struct phy_device *phydev)
{
	int err;
...
static struct phy_driver marvell_drivers[] = {
    ...
    {
		.phy_id = MARVELL_PHY_ID_88E6352,
		.phy_id_mask = MARVELL_88E6352_MASK,
		.name = "Marvell 88E6352",
		.features = PHY_GBIT_FIBRE_FEATURES,
		.probe = m88e6352_probe,
		.config_init = &m88e6352_config_init,
		.config_aneg = &m88e6352_config_aneg,
		.read_status = &m88e6352_read_status,
        .aneg_done = &m88e6352_aneg_done,
        .resume = &m88e6352_resume,
		.suspend = &m88e6352_suspend,
	},
};
...
static struct mdio_device_id __maybe_unused marvell_tbl[] = {
    ...
    { MARVELL_PHY_ID_88E6352, MARVELL_88E6352_MASK },
	{ }
};
