...
bstgmac_probe_config_dt(struct platform_device *pdev, const char **mac)
{
	...
	*mac = of_get_mac_address(np);
	...
	if (dma_cfg->dma_int_mode >= DMA_INT_M_MAX){
		dev_warn(&pdev->dev, "bst,dma_int_mode:%d invalid ,use DMA_INT_M_0\n",dma_cfg->dma_int_mode);
		dma_cfg->dma_int_mode = DMA_INT_M_0;
	}
				  ...
				  struct bstgmac_resources *bstgmac_res)
{
	...
	for(i = 0; i < MTL_MAX_RX_QUEUES;i++){
		...
		bstgmac_res->perch_rx_irq[i] = platform_get_irq_byname(pdev, irq_name);
	...
	for(i = 0; i < MTL_MAX_TX_QUEUES;i++){
		...
		bstgmac_res->perch_tx_irq[i] = platform_get_irq_byname(pdev, irq_name);
