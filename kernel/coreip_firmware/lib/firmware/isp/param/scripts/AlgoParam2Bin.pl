...
open(FH_OUT, ">$out_bin") or die;

binmode(FH_OUT);
my $RegCnt = 0;

my $RegAddr = 0;
my $FirstRegOffsetInit = 0;
my $FirstRegOffset = 0;
my $RegOffset = 0;
my $RegDescrip;
my $RegType;
...
##  print "\n";
    ...
    if($FirstRegOffsetInit == 0){
      $FirstRegOffset = $RegOffset;
      $FirstRegOffsetInit = 1;
    }
    $RegType = $regstr[3];
    seek(FH_OUT, $RegOffset-$FirstRegOffset, SEEK_SET);
    if($regstr[4] =~ m/(0x)/){
