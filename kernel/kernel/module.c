...
void bst_print_modules(char *p)
{
	...
	sprintf(p+strlen(p),"Modules linked in:");
	/* Most callers should already have preempt disabled, but make sure */
	preempt_disable();
	list_for_each_entry_rcu(mod, &modules, list) {
		if (mod->state == MODULE_STATE_UNFORMED)
			continue;
		sprintf(p+strlen(p)," %s%s", mod->name, module_flags(mod, buf));
	}
	preempt_enable();
	if (last_unloaded_module[0])
		sprintf(p+strlen(p)," [last unloaded: %s]", last_unloaded_module);
	sprintf(p+strlen(p),"\n");
}
EXPORT_SYMBOL(bst_print_modules);
