...
void die(const char *str, struct pt_regs *regs, int err)
{
	...
	memset(&die_data, 0, sizeof(die_data));
	die_data.str = str;
	die_data.regs = regs;
	die_data.err = err;
	call_error_notifiers(0x02,&die_data);
...
static void arm64_show_signal(int signo, const char *str)
{
	...
	memset(&user_fault_data,0,sizeof(user_fault_data));
	memcpy(&user_fault_data.regs,regs,sizeof(user_fault_data.regs));
	memcpy(&user_fault_data.comm,tsk->comm,sizeof(user_fault_data.comm));
	user_fault_data.esr = esr;
	user_fault_data.pid = task_pid_nr(tsk);
	user_fault_data.str = str;
	call_error_notifiers(0x09,&user_fault_data);
