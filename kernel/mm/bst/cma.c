...
unsigned long cma_get_free_size(struct cma *cma)
{
...
unsigned long cma_get_largest_free_buf(struct cma *cma)
{
	unsigned long next_zero_bit, next_set_bit;
	unsigned long start = 0;
	unsigned long free_size = 0;
		       ...
		       gfp_t gfp_mask)
{
	...
	int bitmap_len, temp_len, len, pos;
	int first = 1;
	...
	bitmap_len = BITS_TO_LONGS(cma_bitmap_maxno(cma));
	temp_len = (PAGE_SIZE / 8) / 2;
	pos = 0;
	len = bitmap_len < temp_len ? bitmap_len : temp_len;
	while(pos < bitmap_len) {
		if (first) {
			trace_cma_alloc(cma->name, pfn, page, count, align, cma->bitmap, len);
			first = 0;
		} else {
			trace_cma_bitmap(cma->bitmap + pos, len);
		}
		pos += len;
		len = (bitmap_len - pos) < temp_len ? (bitmap_len - pos) : temp_len;
	}
...
bool cma_release(struct cma *cma, const struct page *pages, unsigned int count)
{
	...
	while(pos < bitmap_len) {
		if (first) {
			trace_cma_release(cma->name, pfn, pages, count, cma->bitmap, len);
			first = 0;
		} else {
			trace_cma_bitmap(cma->bitmap + pos, len);
		}
